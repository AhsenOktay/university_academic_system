...
const translations = {
  tr: { title: 'Ödül Ekle', fields: { title: 'Ödül Adı', given_by: 'Veren Kurum', year: 'Yıl', description: 'Açıklama' }, submit: 'Kaydet', cancel: 'İptal', success: 'Ödül başarıyla eklendi!', error: 'Bir hata oluştu.' },
  en: { title: 'Add Award', fields: { title: 'Award Name', given_by: 'Awarding Institution', year: 'Year', description: 'Description' }, submit: 'Save', cancel: 'Cancel', success: 'Award added successfully!', error: 'An error occurred.' }
}

export default function AddAwardPage() {
  const [lang] = useState(() => localStorage.getItem('lang') || 'tr')
  const [loading, setLoading] = useState(false); const [errorMsg, setErrorMsg] = useState(''); const [successMsg, setSuccessMsg] = useState('')
  const navigate = useNavigate(); const tx = translations[lang]
  const titleRef = useRef(); const givenByRef = useRef(); const yearRef = useRef(); const descriptionRef = useRef()

  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!titleRef.current.value) { setErrorMsg(lang === 'tr' ? 'Ödül adı zorunludur.' : 'Award name is required.'); return }
    setLoading(true); setErrorMsg('')
    try {
      const token = localStorage.getItem('access')
      const user = JSON.parse(localStorage.getItem('user') || '{}')
      const res = await fetch('/api/v1/awards/', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ title: titleRef.current.value, given_by: givenByRef.current.value, year: yearRef.current.value ? parseInt(yearRef.current.value) : new Date().getFullYear(), description: descriptionRef.current.value, recipient: user.id }) })
        ...
        <div style={{ background: '#fff', borderRadius: 12, padding: '40px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
              ...
              <div><label style={labelStyle}>{tx.fields.given_by}</label><input ref={givenByRef} type="text" style={inputStyle} onFocus={e => e.target.style.borderColor = '#8B0000'} onBlur={e => e.target.style.borderColor = '#d9d9d9'} /></div>
